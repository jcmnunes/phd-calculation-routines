function [Sobs, Pro, logF, graph] = np_pdna_3c (data, RelTol)
% NP_PDNA_3C  Integration of the Nernst-Planck equation for an
% ionic system with 3 components (pDNA and 2 salt ions) during 
% filtration in Amicon 8010 stirred cell.
%   SOBS = NP_PDNA_3C(DATA) tries to integrate the system of
...
%
%   SOBS is the observed sieving coefficient of the pDNA.
% 
%   Example:
%   % Initiate the cell array DATA (note the use of {} delimiters)
%   data = {6e-6, 316, 760*2*pi/60, 15e-9, 4.47e-6, 6050, 298, ... 
%           'CaCl2'};
%   % call function
%   Sobs = np_pdna_3c(data)
%     
%     Sobs =
%         
%         0.2681
...
% 
%   [SOBS, PRO] = NP_PDNA_3C(DATA) PRO is a structure array 
...
%   plot it along the spatial coordinate, for the 3rd row 
%   values in DATA. 
%     
%     - Extract the salt cation concentration profile
%       from PRO structure: 
%         >> c = PRO(3).cation;
...
%         >> plot(y, c)
%
%   [SOBS, PRO, LOGF] = NP_PDNA_3C(DATA) LOGF is a cell array
...
%
%   [SOBS, PRO, LOGF, GRAPH] = NP_PDNA_3C(DATA) if a 4th output
...
% The 4th output variable does not need allocation (plots)
...
for jj = 1 : len
  ...
  % SOBSOUT (pDNA observed sieving coefficient)
  if condout == 1
    Sobs(jj) = NaN;
